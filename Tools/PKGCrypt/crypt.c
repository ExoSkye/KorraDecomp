...
static PyObject* pkg_crypt(PyObject *self, PyObject *args) {
	...
	long length;
	long remaining, i, offset=0;

	PyObject *arglist;
	PyObject *result;

	if (!PyArg_ParseTuple(args, "s#s#l", &key, &key_length, &input, &input_length, &length))
		return NULL;
